...
| B02 用户资料与关系   | `src/domains/users`         | 当前用户导航卡、正式公开资料头、owner-only 可编辑资料、七字段资料 PATCH、关注/取关/取消请求、入站请求审批、followers/following cursor 列表、owner-only 静音/屏蔽管理列表与解除操作 | Refresh 后以 `/api/users/me` 水合导航用户；关系列表严格使用 canonical relationship snapshot；资料媒体只提交 B05 READY 后的 storage key |
