...
| 乐观布尔写操作               | `src/shared/hooks/useOptimisticBooleanMutation.ts`                      | 单一布尔开关优先复用；跨页面帖子互动使用统一 feature                          |
...
| 双图片选择                   | `src/domains/media/hooks/useMediaImagePairSelection.ts`                 | 同时管理头像/封面的页面复用该组合 Hook                                        |
...
| 页面标题                     | `src/shared/ui/PageHeader`                                              | 直接使用 PageHeader，旧 PageTitle 别名已移除                                  |

发帖流程唯一入口为 `src/features/compose-post/index.ts`；卡片和评论的互动行为唯一入口为 `src/features/post-interactions/index.ts`。媒体协议和帖子合同仍由相应 domains 持有。
