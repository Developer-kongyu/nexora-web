...
import { isCommunityManageSection, type CommunityManageSection } from './communityManage.model';
import { ContentAvailabilitySection } from './sections/ContentAvailabilitySection';
...
function renderSection(
  ...
  switch (active) {
    ...
    case 'members':
      return <MembersSection communityId={communityId} />;
    case 'content':
      return <ContentAvailabilitySection />;
    case 'pinned':
...
export function CommunityManagePage() {
  ...
  const active: CommunityManageSection = isCommunityManageSection(params.section)
    ? params.section
    : 'overview';
    ...
    return (
      <>
        ...
        <Card>
          <EmptyPanel
            title="无法加载管理台"
            description="没有使用硬编码社群资料替代失败响应。请确认社群存在且网络正常。"
  ...
  if (!detailData.viewerContext?.canManageCommunity) {
    return (
      <>
        ...
        <Card>
          <EmptyPanel
            icon={<ShieldAlert size={30} />}
            title="没有管理权限"
            description="当前账号不能管理该社群。页面不会展示或模拟管理数据。"
            action={
  ...
  return (
    <>
      ...
      <div className={styles.layout}>
        ...
        />
        <main>{renderSection(active, communityId, detail.data, switchSection)}</main>
      </div>
    </>
  );
}
